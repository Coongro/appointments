# appointments - Development Guide
...
- **Trigger name**: `trg_appointments_{descriptive_name}`
- **Function name**: `trg_appointments_{descriptive_name}_fn`

Where `appointments` is your plugin's moduleId (pluginId with hyphens replaced by underscores).
...
CREATE OR REPLACE FUNCTION "trg_appointments_audit_log_fn"()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER "trg_appointments_audit_log"
AFTER INSERT OR UPDATE ON "module_appointments_items"
FOR EACH ROW
EXECUTE FUNCTION "trg_appointments_audit_log_fn"();
```

> **Important**: The system automatically **DISABLE**s all `trg_appointments_*` triggers when the plugin
